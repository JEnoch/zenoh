...
impl FilesMgr {
    pub(crate) fn new(base_dir: &str, follow_links: bool, on_closure: OnClosure) -> Self {
    ...
    pub(crate) fn read_file<P: AsRef<Path>>(
        ...
        match File::open(relative_path) {
                ...
                if size <= usize::MAX as u64 {
                    ...
                    if let Err(e) = f.read_to_end(&mut content) {
                        ...
                        let (timestamp, encoding) =
                            self.get_encoding_and_timestamp(relative_path.to_str().unwrap())?;
            ...
            Err(e) => zerror!(ZErrorKind::Other {
                descr: format!(r#"Error reading file "{}": {}"#, relative_path.display(), e)
            }),
        }
    }
    ...
    fn get_timestamp(&self, file: &str) -> ZResult<Timestamp> {
        ...
        // fallback: get timestamp from file's metadata
        self.get_timestamp_from_metadata(file)
    }
